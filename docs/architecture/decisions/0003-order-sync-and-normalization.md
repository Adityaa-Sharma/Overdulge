# ADR-0003: Order sync pipeline — MCP client, adapter/normalize split, and sync orchestration
...
Token refresh (ADR-0001: "on sync or on-demand platform calls") lives in
`oauth/engine.py`, which FR-1 is still building (#17-#20). FR-2's own tasks
must not re-implement refresh logic or block entirely on FR-1 finishing —
only the orchestration task that actually calls refresh needs to wait on it.
...
- The **Cron Trigger entrypoint** loops `run_sync_for_account` over every
  `linked_accounts` row (service-role client, ADR-0002).
