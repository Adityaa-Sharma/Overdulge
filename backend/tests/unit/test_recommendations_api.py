"""Unit tests for `GET /api/v1/recommendations/usuals` (#40, ADR-0004).

Mocks `linked_accounts`, `engine.resolve_access_token`, and the adapter
`get_usual_items` functions (their own behaviour is covered by the adapter
unit tests and `test_mcp_recommendations.py`) — this file only asserts the
route's own contract: response shape, the Zepto join key (AC-2), Food
grouping/ranking, the no-account/no-orders empty-list paths, and that no
live search tool is ever called from this route.
"""
...
def no_live_search_calls(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    `search_products`/`search_menu` call fails the test loudly.
    """
...
def test_instamart_usuals_key_on_item_id(monkeypatch: pytest.MonkeyPatch, fake_user_client) -> None:
    monkeypatch.setattr(
        recommendations_module.linked_accounts,
        "get_linked_account",
        lambda client, *, user_id, platform: (
